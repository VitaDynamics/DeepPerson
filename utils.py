...
def save_gallery_index(
    ...
    """
    Save gallery index (FAISS or sklearn) with metadata.

    This function integrates with FAISSSearcher and SklearnSearcher to persist
    gallery data including index, embeddings, and metadata.

    Args:
        searcher: FAISSSearcher or SklearnSearcher instance
        gallery_dir: Directory to save gallery files
        gallery_name: Base name for gallery files
        model_profile_id: Optional model profile identifier
    ...
    Examples:
        ...
        >>> save_gallery_index(searcher, Path("./galleries"), "my_gallery")
    """
    gallery_dir = Path(gallery_dir)
    gallery_dir.mkdir(parents=True, exist_ok=True)

    # Create subdirectory for this gallery
    gallery_path = gallery_dir / gallery_name
    gallery_path.mkdir(parents=True, exist_ok=True)

    # Save the searcher index and metadata
    searcher.save(str(gallery_path))
    ...
    enhanced_manifest = {
        ...
        "feature_dimension": searcher.dimension if hasattr(searcher, 'dimension') else None,
        "created_at": str(Path(gallery_path).stat().st_mtime) if gallery_path.exists() else None
    }
...
def load_gallery_index(
    ...
    """
    Load gallery index (FAISS or sklearn) from disk.

    Args:
        gallery_dir: Directory containing gallery files
        gallery_name: Base name for gallery files
        backend: Search backend to use ('auto', 'faiss', 'sklearn')
        device: Device for FAISS ('cpu' or 'cuda')

    Returns:
        Loaded searcher instance (FAISSSearcher or SklearnSearcher)

    Raises:
        FileNotFoundError: If gallery files not found
        ValueError: If gallery data is invalid

    Examples:
        >>> searcher = load_gallery_index(Path("./galleries"), "my_gallery")
        >>> results = searcher.search(query_embedding, k=10)
    """
    gallery_path = Path(gallery_dir) / gallery_name

    if not gallery_path.exists():
        raise FileNotFoundError(f"Gallery not found: {gallery_path}")

    # Load enhanced manifest if exists
    manifest_path = gallery_path / "gallery_manifest.json"
    if manifest_path.exists():
        with open(manifest_path, 'r') as f:
            manifest = json.load(f)
        logger.info(f"Loading gallery '{gallery_name}': {manifest.get('total_entries', 0)} entries")
        metric = manifest.get("metric", "cosine")
        dimension = manifest.get("feature_dimension", 2048)
    else:
        # Fall back to basic metadata
        basic_manifest_path = gallery_path / "metadata.json"
        if basic_manifest_path.exists():
            with open(basic_manifest_path, 'r') as f:
                basic_manifest = json.load(f)
            metric = basic_manifest.get("metric", "cosine")
            dimension = basic_manifest.get("dimension", 2048)
        else:
            raise FileNotFoundError(f"No manifest found in {gallery_path}")
    ...
    # Load saved data
    searcher.load(str(gallery_path))

    logger.info(f"Loaded gallery '{gallery_name}' from {gallery_path}")
    return searcher
